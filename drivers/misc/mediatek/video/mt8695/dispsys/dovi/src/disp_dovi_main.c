...
{
	if (res >= HDMI_VIDEO_RESOLUTION_NUM || res < 0)
		return DOVI_STATUS_ERROR;
...
uint32_t disp_dovi_set_idk_info(void)
{
	if (dovi_idk_test) {
		...
		if (dovi_res.res_mode < 0 || dovi_res.res_mode >= HDMI_VIDEO_RESOLUTION_NUM)
			return DOVI_STATUS_ERROR;
...
uint32_t disp_dovi_set_osd_showdoblylogo(void)
{
	...
	uint32_t width = 0;
	uint32_t height = 0;
	unsigned int osd_hstart = 0;
	unsigned int osd_veven = 0;
	unsigned int osd_vodd = 0;
