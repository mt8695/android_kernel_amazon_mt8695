...
void disp_dovi_idk_dump_frame_start(uint32_t file_id)
{
	if (!dump_file_opened) {
		...
		if (ll_rgb_desired)
			ret = sprintf(fileName, "/sdcard/dump_%d_frame.rgb",
			file_id);
		else
			ret = sprintf(fileName, "/sdcard/dump_%d_frame.yuv",
			file_id);
...
void disp_dovi_dump_buffer(char *file_name, unsigned char *buff, uint32_t len)
{
	...
	if (file_name != NULL && *file_name != '\0') {
		ret = sprintf(fileName, "/sdcard/dovi/%s", file_name);
		if (ret <= 0)
			return;
	}
...
unsigned char *buff, uint32_t len)
{
	...
	memset(fileName, 0, MAX_FILE_NAME);
	ret = sprintf(fileName, "tmp_comp.%04u.bin", frame_num);
	if (ret <= 0)
		return;
...
unsigned char *buff, uint32_t len)
{
	...
	memset(fileName, 0, MAX_FILE_NAME);
	ret = sprintf(fileName, "tmp_dm.%04u.bin", frame_num);
	if (ret <= 0)
		return;
...
unsigned char *buff, uint32_t len)
{
	...
	memset(fileName, 0, MAX_FILE_NAME);
	ret = sprintf(fileName, "tmp_hdmi_dm.%04u.bin", frame_num);
	if (ret <= 0)
		return;
...
uint32_t len)
{
	...
	memset(fileName, 0, MAX_FILE_NAME);
	ret = sprintf(fileName, "tmp_dm_core%u_reg.%04u.bin", core_id,
		frame_num);
	if (ret <= 0)
		return;
...
uint32_t len)
{
	...
	memset(fileName, 0, MAX_FILE_NAME);
	ret = sprintf(fileName, "tmp_dm_core%u_lut.%04u.bin", core_id,
		frame_num);
	if (ret <= 0)
		return;
...
enum dovi_clr_fmt color_fmt)
{
	...
	if (allocated[vdp_id]) {
		...
		ret = sprintf(y_file_name, "/sdcard/vdp/%d/%s", pattern_idx[pattern],
		"pic_0_Y.out");
		if (ret <= 0)
			return;
		ret = sprintf(c_file_name, "/sdcard/vdp/%d/%s", pattern_idx[pattern],
		"pic_0_CbCr.out");
		if (ret <= 0)
			return;
