...
int disp_osd_dump_plane(unsigned int plane)
{
...

	{
		...
		strncpy(file_name, bmp_name, sizeof(bmp_name));
		filp = filp_open(file_name, O_CREAT | O_RDWR, 0);
		...
		if (IS_ERR(filp)) {
			...
			i4WriteSize = filp->f_op->write(filp, (UINT8 *)((&t_bmpinfo) + 2),
...
static void disp_osd_process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "log_level:", 10) == 0) {
	...
	} else if (strncmp(opt, "dump_info:", 10) == 0) {
		unsigned int value = 0;
		...
		disp_osd_dump(value);
	} else if (strncmp(opt, "faker_hdr:", 10) == 0) {
		unsigned int value = 0;
