...
static struct imgresz_data *imgresz_get_data(const IMGRESZ_TICKET ti)
{
	if ((!imgresz_valid_ticket(ti)) ||
	    (TICKET_TO_HWID(ti) >= imgresz_hw_num)) {
		logwarn("%s invalid ticket:0x%x\n", __func__, ti);
		return NULL;
	}
...
/* Be sure called after imgresz_get_ticket()*/
int imgresz_clk_on(unsigned int hw_id)
{
	struct imgresz_data *data;
	int ret = 0;

	data = imgresz_get_data(TICKET_FROM_HWID(hw_id));
	mtk_smi_larb_get(data->smi_larb_dev);
	pm_runtime_get_sync(&data->pdev->dev);
	ret = imgresz_clk_enable(data);
	if (ret)
		logwarn("hw%d clk enable fail %d\n", hw_id, ret);

	return ret;
}

void imgresz_clk_off(unsigned int hw_id)
{
	struct imgresz_data *data;
...
static int _imgresz_hal_if_do_scale(struct imgresz_data *data)
{
	...
	struct imgresz_partition_info *partition = &scaledata->partition;
...
*/
	if (imgresz_src_is_ufo(src_buf->ufo_type)) {
		...
		imgresz_ufo_pagesz(base, partition->src_w, src_buf->pic_height, partition->dst_w);
	...
	imgresz_hal_set_src_pic_wid_hei(base,
		partition->src_w_y, partition->src_w_c, partition->src_w_cr,
		partition->src_h_y, partition->src_h_c, partition->src_h_cr);
	imgresz_hal_set_src_pic_offset(base, scaledata);
	...
	imgresz_hal_set_dst_pic_wid_hei(base, partition->dst_w, dst_buf->pic_height);
	imgresz_hal_set_dst_pic_offset(base, dst_buf->pic_x_offset + partition->dst_x_offset, dst_buf->pic_y_offset);
	...
	imgresz_hal_set_linebuflen(
			...
			partition->src_w, src_buf->pic_height,
			partition->dst_w, dst_buf->pic_height,
	...
	*/
	if (h_partition && imgresz_src_is_ufo(src_buf->ufo_type)) {
		if (scaledata->ufo_page0)
			...
			imgresz_hal_coeff_h8tap_vdo_partition_offset(base, partition->dst_x_offset,
				hal_info->h8_factor_y, hal_info->h8_factor_cb);
			/*imgresz_hal_coeff_v4tap_vdo_partition_offset(base, 0);*/
			imgresz_ufo_partition_set_start_point(base, partition->src_x_offset, 0);
		}
...
static int imgresz_ufo_partition_init(struct imgresz_scale_data *scaledata)
{
	struct imgresz_buf_format *src_format = &scaledata->src_format;
	struct imgresz_partition_info *partition = &scaledata->partition;

	partition->dst_w = scaledata->dst_buf.pic_width;
	partition->src_w = partition->src_w_y = scaledata->src_buf.pic_width;
	partition->src_h = partition->src_h_y = scaledata->src_buf.pic_height;

	switch (src_format->mainformat) {
	case IMGRESZ_BUF_MAIN_FORMAT_Y_C:
		switch (src_format->yuv_format) {
		case IMGRESZ_YUV_FORMAT_420:
			partition->src_w_c = partition->src_w_y/2;
			partition->src_h_c = partition->src_h_y/2;
			break;
		case IMGRESZ_YUV_FORMAT_422:
			partition->src_w_c = partition->src_w_y/2;
			partition->src_h_c = partition->src_h_y;
			break;
		case IMGRESZ_YUV_FORMAT_444:
			partition->src_w_c = partition->src_w_y;
			partition->src_h_c = partition->src_h_y;
			break;
		default:
			return -EINVAL;
	...
	case IMGRESZ_BUF_MAIN_FORMAT_Y_CB_CR:
		if ((src_format->h_sample[1] != 0) && (src_format->v_sample[1] != 0)) {
			partition->src_w_c = partition->src_w_y * src_format->h_sample[1] / src_format->h_sample[0];
			partition->src_h_c = partition->src_h_y * src_format->v_sample[1] / src_format->v_sample[0];
			/* For jpeg picture mode, prevent source height 401 come
			 * two interrupt (Y interrupt and C interrupt)
			 */
			if ((partition->src_h_c * src_format->v_sample[0]) !=
			    (partition->src_h_y * src_format->v_sample[1]))
				partition->src_h_c++;
		}

		if ((src_format->h_sample[2] != 0) && (src_format->v_sample[2] != 0)) {
			partition->src_w_cr = partition->src_w_y * src_format->h_sample[2] / src_format->h_sample[0];
			partition->src_h_cr = partition->src_h_y * src_format->v_sample[2] / src_format->v_sample[0];
			/* For jpeg picture mode, prevent source height 401 come
			 * two interrupt (Y interrupt and C interrupt)
			 */
			if ((partition->src_h_cr * src_format->v_sample[0]) !=
			    (partition->src_h_y * src_format->v_sample[2]))
				partition->src_h_cr++;
		}
		break;
	case IMGRESZ_BUF_MAIN_FORMAT_ARGB:
		...
		struct imgresz_scale_data *scaledata_2nd)
{
	...
	struct imgresz_buf_format *dst_format = &scaledata->dst_format;
	struct imgresz_hal_info *hal_info = &scaledata->hal_info;
	struct imgresz_partition_info *partition0 = &scaledata->partition;
	struct imgresz_partition_info *partition1 = &scaledata_2nd->partition;
	unsigned int x1, x1_c;
	unsigned int x0_end, x0_c_end;
	unsigned int tg_x1, u4Count;
	bool fgLinebufok;
	...
	partition0->src_w = partition0->src_w_y = x0_end;
	partition0->src_w_c = x0_end/2;
	partition0->dst_x_offset = 0;
	partition0->dst_w = tg_x1;
	partition0->src_h = partition0->src_h_y = src_buf->pic_height;
	partition0->src_h_c = src_buf->pic_height/2;

	partition1->src_x_offset = x1;
	partition1->src_x_offset_c = x1_c;
	partition1->src_w_y = src_buf->pic_width - x1;
	partition1->src_w_c = src_buf->pic_width / 2 - x1_c;
	partition1->src_w = max(partition1->src_w_y, 2 * partition1->src_w_c);
	partition1->dst_x_offset = tg_x1;
	partition1->dst_w = dst_buf->pic_width - tg_x1;
	partition1->src_h = partition1->src_h_y = src_buf->pic_height;
	partition1->src_h_c = src_buf->pic_height/2;

	loginfo(IMGRESZ_LOG_UFO, "p0: 0~%d->0~%d; p1: %d(c:%d)+%d(y:%d,c:%d)->%d+%d\n",
		partition0->src_w, partition0->dst_w, partition1->src_x_offset,
		partition1->src_x_offset_c, partition1->src_w,
		partition1->src_w_y, partition1->src_w_c,
...
static void imgresz_prepare_param(struct imgresz_data *data, struct imgresz_data *data_2nd)
{
	...
	if (scaledata->ufo_h_partition && data_2nd) {
		...
		imgresz_ufo_partition_init(scaledata);
	}
}
...
int imgresz_trigger_scale_async(IMGRESZ_TICKET ti, bool async)
{
	...
	if (scaledata->src_buf.buf_info.mem_type == IMGRZ_MEM_SECURE) {
		...
		if (data->state == IMGRESZ_STATE_DONE) {
			if (data->cb_info.fun_cb)
				data->cb_info.fun_cb(TICKET_FROM_HWID(data->hw_id),
					IMGRESZ_CB_DONE, data->cb_info.cb_data);
			goto DONE;
		}
		goto TIMEOUT;
	}
