...
#define ARMPLL_CON0		(APMIXED_BASE + 0x0110)
#define ARMPLL_CON1		(APMIXED_BASE + 0x0114)
#define ARMPLL_CON2		(APMIXED_BASE + 0x0118)
#define ARMPLL_PWR_CON0	(APMIXED_BASE + 0x011C)

#define MMPLL_CON0		(APMIXED_BASE + 0x0140)
#define MMPLL_CON1		(APMIXED_BASE + 0x0144)
#define MMPLL_CON2		(APMIXED_BASE + 0x0148)
#define MMPLL_PWR_CON0		(APMIXED_BASE + 0x014C)

#define MAINPLL_CON0		(APMIXED_BASE + 0x0120)
#define MAINPLL_CON1		(APMIXED_BASE + 0x0124)
#define MAINPLL_CON2		(APMIXED_BASE + 0x0128)
#define MAINPLL_PWR_CON0	(APMIXED_BASE + 0x012C)

#define VDECPLL_CON0		(APMIXED_BASE + 0x0190)
#define VDECPLL_CON1		(APMIXED_BASE + 0x0194)
#define VDECPLL_CON2		(APMIXED_BASE + 0x0198)
#define VDECPLL_PWR_CON0	(APMIXED_BASE + 0x019C)

#define MSDCPLL_CON0		(APMIXED_BASE + 0x0150)
#define MSDCPLL_CON1		(APMIXED_BASE + 0x0154)
#define MSDCPLL_CON2		(APMIXED_BASE + 0x0158)
#define MSDCPLL_PWR_CON0	(APMIXED_BASE + 0x015C)

#define TVDPLL_CON0		(APMIXED_BASE + 0x0170)
#define TVDPLL_CON1		(APMIXED_BASE + 0x0174)
#define TVDPLL_CON2		(APMIXED_BASE + 0x0178)
#define TVDPLL_PWR_CON0	(APMIXED_BASE + 0x017C)
...
#define MASKDIV (0x7000000u)
#define MASK21b (0x3FFFFFu)
#define BIT31   (1u<<31u)
...
static void __iomem *g_reg_pll_con0[FH_PLL_NUM];
...
static void fh_sync_ncpo_to_fhctl_dds(unsigned int pll_id)
{
	...
	if ((pll_id == FH_ARM_PLLID) || (pll_id == FH_MM_PLLID)) {
		/* MT8695: ARMPLL/MMPLL use the same bits as pcw, do nothing */
		/* pll_con1 = pll_con1; */
	} else if (pll_id != FH_MEM_PLLID) {
		/* MT8695: To apmixed: we only extract significant bits(31:10) from apmixed's pll to
		 * fhctl dds register.
		 */
		pll_con1 = pll_con1 >> 10;
	} else {
		/* MT8695: To mempll: MEMPLL uses high 16 bits(31:16). Therefore, we extract its 31:16 bits.
	...
	fh_write32(g_reg_dds[pll_id], (pll_con1 & MASK21b) | BIT31);
}
...
static void __enable_ssc(unsigned int pll_id)
{
	...
	fh_write32(reg_updnlmt,
		   (percent_to_ddslmt((fh_read32(reg_dds) & MASK21b), 8u) << 16u));
...
static int mt_fh_dumpregs_proc_read(struct seq_file *m, void *data)
{
	...
	for (i = 0; i < FH_PLL_NUM; ++i) {
		/* mt8695  doesn't need to support MEMPLL. Therefore, ignore MEMPLL setting. */
		if (i == FH_MEM_PLLID)
			continue;
	...
	seq_puts(m, "\r\nPLL_CON0 :\r\n");
	seq_printf(m, "ARM:0x%08x\nMM:0x%08x\nMAIN:0x%08x\n",
		   fh_read32(ARMPLL_CON0), fh_read32(MMPLL_CON0),
		   fh_read32(MAINPLL_CON0));
	seq_printf(m, "VDEC:0x%08x\nMSDC:0x%08x\nTVD:0x%08x\n\n",
		   fh_read32(VDECPLL_CON0), fh_read32(MSDCPLL_CON0),
		   fh_read32(TVDPLL_CON0));

	seq_puts(m, "\r\nPLL_CON1 :\r\n");
	seq_printf(m, "ARM:0x%08x\nMM:0x%08x\nMAIN:0x%08x\n",
		   fh_read32(ARMPLL_CON1), fh_read32(MMPLL_CON1),
		   fh_read32(MAINPLL_CON1));
	seq_printf(m, "VDEC:0x%08x\nMSDC:0x%08x\nTVD:0x%08x\n\n",
		   fh_read32(VDECPLL_CON1), fh_read32(MSDCPLL_CON1),
...
{
	g_reg_pll_con0[FH_ARM_PLLID] = ARMPLL_CON0;
	g_reg_pll_con1[FH_ARM_PLLID] = ARMPLL_CON1;
	g_reg_dds[FH_ARM_PLLID] = REG_FHCTL0_DDS;
	g_reg_cfg[FH_ARM_PLLID] = REG_FHCTL0_CFG;
	g_reg_updnlmt[FH_ARM_PLLID] = REG_FHCTL0_UPDNLMT;
	g_reg_mon[FH_ARM_PLLID] = REG_FHCTL0_MON;
	g_reg_dvfs[FH_ARM_PLLID] = REG_FHCTL0_DVFS;
	g_slt_fmax[FH_ARM_PLLID] = 0x208000;
	g_slt_fmin[FH_ARM_PLLID] = 0x1CC000;

	g_reg_pll_con0[FH_MM_PLLID] = MMPLL_CON0;
	g_reg_pll_con1[FH_MM_PLLID] = MMPLL_CON1;
	g_reg_dds[FH_MM_PLLID] = REG_FHCTL1_DDS;
	g_reg_cfg[FH_MM_PLLID] = REG_FHCTL1_CFG;
	g_reg_updnlmt[FH_MM_PLLID] = REG_FHCTL1_UPDNLMT;
	g_reg_mon[FH_MM_PLLID] = REG_FHCTL1_MON;
	g_reg_dvfs[FH_MM_PLLID] = REG_FHCTL1_DVFS;
	g_slt_fmax[FH_MM_PLLID] = 0x248000;
	g_slt_fmin[FH_MM_PLLID] = 0x134000;

	g_reg_pll_con0[FH_MAIN_PLLID] = MAINPLL_CON0;
	...
	g_reg_pll_con0[FH_VDEC_PLLID] = VDECPLL_CON0;
	g_reg_pll_con1[FH_VDEC_PLLID] = VDECPLL_CON1;
	g_reg_dds[FH_VDEC_PLLID] = REG_FHCTL3_DDS;
	g_reg_cfg[FH_VDEC_PLLID] = REG_FHCTL3_CFG;
	g_reg_updnlmt[FH_VDEC_PLLID] = REG_FHCTL3_UPDNLMT;
	g_reg_mon[FH_VDEC_PLLID] = REG_FHCTL3_MON;
	g_reg_dvfs[FH_VDEC_PLLID] = REG_FHCTL3_DVFS;
	g_slt_fmax[FH_VDEC_PLLID] = 0x200000;
	g_slt_fmin[FH_VDEC_PLLID] = 0x168000;

	g_reg_pll_con0[FH_MSDC_PLLID] = MSDCPLL_CON0;
	...
	g_reg_pll_con0[FH_TVD_PLLID] = TVDPLL_CON0;
	g_reg_pll_con1[FH_TVD_PLLID] = TVDPLL_CON1;
	g_reg_dds[FH_TVD_PLLID] = REG_FHCTL5_DDS;
	g_reg_cfg[FH_TVD_PLLID] = REG_FHCTL5_CFG;
	g_reg_updnlmt[FH_TVD_PLLID] = REG_FHCTL5_UPDNLMT;
	g_reg_mon[FH_TVD_PLLID] = REG_FHCTL5_MON;
	g_reg_dvfs[FH_TVD_PLLID] = REG_FHCTL5_DVFS;
	g_slt_fmax[FH_TVD_PLLID] = 0x1CC000;
	g_slt_fmin[FH_TVD_PLLID] = 0x140000;

	g_reg_pll_con1[FH_MEM_PLLID] = MEMPLL; /* FHCTL doesn't need to support MEMPLL */
	g_reg_dds[FH_MEM_PLLID] = REG_FHCTL6_DDS;
	g_reg_cfg[FH_MEM_PLLID] = REG_FHCTL6_CFG;
	g_reg_updnlmt[FH_MEM_PLLID] = REG_FHCTL6_UPDNLMT;
	g_reg_mon[FH_MEM_PLLID] = REG_FHCTL6_MON;
	g_reg_dvfs[FH_MEM_PLLID] = REG_FHCTL6_DVFS;
...
static int mt_fh_init(void)
{
	...
	freqhopping_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8695-fhctl");
	if (freqhopping_node == NULL) {
		FH_MSG_ERROR("node \"mediatek,mt8695-fhctl\" not found!\n");
		return -ENODEV;
	}
	...
	apmix_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8695-apmixedsys");
	if (apmix_node == NULL) {
		FH_MSG_ERROR("node \"mediatek,mt8695-apmixedsys\" not found!\n");
		return -ENODEV;
	}
...
int mtk_fhctl_hopping_by_id(unsigned int fh_pll_id, unsigned int target_vco_frequency)
{
	...
	/* MT8695:
	 * ARMPLL/MMPLL: FHCTL_MON use the same bits as apmixed, pcw_chg register is con1
	 * Others(except MEMPLL): APMIXED PLL uses bit 31:0 but FHCTL_MON uses 21:0, pcw_chg register is con0.
	 */
	if ((fh_pll_id == FH_ARM_PLLID) || (fh_pll_id == FH_MM_PLLID)) {
		fh_vco_freq = ((fh_read32(g_reg_pll_con1[fh_pll_id]) & MASKDIV)) |
			((fh_read32(g_reg_mon[fh_pll_id]) & MASK21b)) | (BIT31);
		/* write back to ncpo */
		fh_write32(g_reg_pll_con1[fh_pll_id], fh_vco_freq);
	} else if (fh_pll_id != FH_MEM_PLLID) {
		fh_vco_freq = ((fh_read32(g_reg_mon[fh_pll_id]) & MASK21b) << 10);
		 /* write back to ncpo */
		fh_write32(g_reg_pll_con1[fh_pll_id], fh_vco_freq);
		fh_write32(g_reg_pll_con0[fh_pll_id], (fh_read32(g_reg_pll_con0[fh_pll_id]) | BIT31));
	}
...
static int test_freqhopping(unsigned int fh_pll_id, char *fh_pll_name)
{
	...
	FH_MSG_NOTICE("[%s]DDS: 0x%08x\n", fh_pll_name, (fh_read32(g_reg_dds[fh_pll_id]) & MASK21b));
	fh_write32(g_reg_dvfs[fh_pll_id], (g_slt_fmin[fh_pll_id]) | (BIT31));	/* set hopping dvfs */
	FH_MSG_NOTICE("[%s]DVFS: 0x%08x\n", fh_pll_name, (fh_read32(g_reg_dvfs[fh_pll_id]) & MASK21b));
...
static int test_ssc(unsigned int fh_pll_id, char *fh_pll_name)
{
	...
	dds_value = fh_read32(g_reg_dds[fh_pll_id]) & MASK21b;
	...
	fh_write32(g_reg_updnlmt[fh_pll_id],
			(percent_to_ddslmt((fh_read32(g_reg_dds[fh_pll_id]) & MASK21b), 8u) << 16u));
	...
	for (count = 0; count <= 3000; count++) {
		mon_value = fh_read32(g_reg_mon[fh_pll_id]) & MASK21b;
...
static int mt_fh_slt_start(void)
{
	...
	for (fh_pll_id = FH_ARM_PLLID; fh_pll_id < FH_PLL_NUM; fh_pll_id++) {
