...
void sdio_set_card_clkpd(int on)
{
	...
	if (!on) {
		...
		pr_err("sdio host MSDC_CLK_CKPDN value: %d", value);
	} else {
...
static void msdc_save_reg(struct msdc_host *host)
{
	...
	host->save_para.sdc_adv_cfg = readl(host->base + SDC_ADV_CFG0);
}
...
static void msdc_restore_reg(struct msdc_host *host)
{
	...
	writel(host->save_para.sdc_adv_cfg, host->base + SDC_ADV_CFG0);
}
