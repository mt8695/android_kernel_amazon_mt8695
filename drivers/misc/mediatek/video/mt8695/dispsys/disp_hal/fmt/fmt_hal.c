...
void fmt_hal_enable(uint32_t fmt_id, bool enable)
{
	...
	FMT_LOG_D("enable fmt, fmt_id: %d, enable: %d\n", fmt_id, enable);
...
int32_t fmt_hal_h_down_enable(uint32_t fmt_id, struct fmt_hd_scl_info *info)
{
	...
	if ((info->src_w < 1920) || ((info->src_w == 1920) && (fmt_id == DISP_FMT_MAIN)))
	...
	if (fmt_id == VDOUT_FMT) {
		...
		if (id == DISP_FMT1) {
			...
			if (on) {
				...
				disp_fmt = fmt_hal_get_dispfmt_reg(DISP_FMT1);
				if (disp_fmt->status == FMT_STA_UNUSED) {
