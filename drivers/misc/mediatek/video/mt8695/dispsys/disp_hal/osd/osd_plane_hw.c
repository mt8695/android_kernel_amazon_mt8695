...
INT32 osd_plane_update_extend(UINT32 u4Plane)
{
	...
	/*handle 40,50 ,D0 offset registers for the faker header function*/
	for (u4Idx = 0x10; u4Idx < 0x18; u4Idx++) {
		_prHwOsdPlaneCoreReg[u4Plane]->au4Reg[u4Idx] =
		    _rOsdPlaneCoreReg[u4Plane].au4Reg[u4Idx];
		OSD_PRINTF(OSD_WRITE_HW_LOG, "PLA_WRITE(%d + 'h%x, 32'h%x)\n", u4Plane, (u4Idx*4),
			_prHwOsdPlaneCoreReg[u4Plane]->au4Reg[u4Idx]);
	}
	for (u4Idx = 0x34; u4Idx < 0x38; u4Idx++) {
		_prHwOsdPlaneCoreReg[u4Plane]->au4Reg[u4Idx] =
		    _rOsdPlaneCoreReg[u4Plane].au4Reg[u4Idx];
		OSD_PRINTF(OSD_WRITE_HW_LOG, "PLA_WRITE(%d + 'h%x, 32'h%x)\n", u4Plane, (u4Idx*4),
			_prHwOsdPlaneCoreReg[u4Plane]->au4Reg[u4Idx]);
...
int osd_pla_set_faker_header_register(UINT32 u4Plane, OSD_RGN_UNION_T *prRgn)
{
	INT32 i;

	for (i = 0; i < 12; i++)
		_rOsdPlaneCoreReg[u4Plane].au4Reg[16+i] = prRgn->au4Reg[i];
	for (i = 0; i < 4; i++)
		_rOsdPlaneCoreReg[u4Plane].au4Reg[52+i] = prRgn->au4Reg[8+i];
	return (INT32) OSD_RET_OK;
}
