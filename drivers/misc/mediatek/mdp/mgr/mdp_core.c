...
enum MDP_TASK_STATUS mdp_core_copy_mmp_data(struct mdp_task_struct *pTask, struct mmp_mdp_task_struct *pMmpData)
{
	...
	pMmpData->src_release_fence = pTask->src_fence.fence.fence;
	pMmpData->dst_release_fence = pTask->dst_fence.fence.fence;
...
void mdp_core_init_mdp_task(struct mdp_task_struct *pTask)
{
	...
	memset(&pTask->src_fence, 0, sizeof(pTask->src_fence));
	memset(&pTask->dst_fence, 0, sizeof(pTask->dst_fence));
}
...
enum MDP_TASK_STATUS mdp_core_put_mdp_task(struct mdp_task_struct *pTask)
{
	do {
		if (mdp_proc_get_time_in_us(pTask->time.start, pTask->time.releaseFence) > 16670) {
			...
			size = snprintf(show_buffer, sizeof(show_buffer),
...
void mdp_core_release_task(struct work_struct *work)
{
	...
	/* disable clk */
	mdp_imgresz_mgr_task_clk(pTask, false);
...
enum MDP_TASK_STATUS mdp_core_handle_exec_command(struct mdp_command_struct *pCommand)
{
	...
	if (pTask == NULL) {
		MDP_ERR("mdp get task fail\n");
		return MDP_TASK_STATUS_GENERAL_ERROR;
	}
	...
	if (status != MDP_TASK_STATUS_OK) {
		...
		goto ACQUIRE_FENCE_ERR;
	}

	/* copy created fence fd to mdp_command_struct */
	pCommand->src_buffer.release_fence_fd = pTask->src_fence.fence.fence;
	pCommand->dst_buffer.release_fence_fd = pTask->dst_fence.fence.fence;
