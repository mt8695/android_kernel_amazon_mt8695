...
bool mdp_fence_is_first_fence_in_list(struct mdp_fence_struct *pFence)
{
	...
	if (pFence == NULL || pFence->type < 0 || pFence->type >= MDP_TIMELINE_TYPE_MAX) {
...
enum MDP_TASK_STATUS mdp_fence_get_task_fence(struct mdp_task_struct *pTask)
{
	...
	int size = 0;
	#if 0
	struct mdp_fence_struct *p_src_fence = vmalloc(sizeof(struct mdp_fence_struct));
	struct mdp_fence_struct *p_dst_fence = vmalloc(sizeof(struct mdp_fence_struct));
	#else
	struct mdp_fence_struct *p_src_fence = &pTask->src_fence;
	struct mdp_fence_struct *p_dst_fence = &pTask->dst_fence;
	#endif
...
#if 0
	enum MDP_TIMELINE_TYPE src_timeline_type = MDP_TIMELINE_TYPE_IMGRESZ_NORMAL;
	enum MDP_TIMELINE_TYPE dst_timeline_type = MDP_TIMELINE_TYPE_IMGRESZ_NORMAL;
	...
	size = snprintf(p_src_fence->taskString, sizeof(p_src_fence->taskString), "%s", pTask->mdp_command.taskString);
	if (size < 0)
		MDP_ERR("fill buffer fail:%d\n", __LINE__);
	
	size = snprintf(p_dst_fence->taskString, sizeof(p_dst_fence->taskString), "%s", pTask->mdp_command.taskString);
	if (size < 0)
			MDP_ERR("fill buffer fail:%d\n", __LINE__);
	...
	size = snprintf(p_src_fence->fence.name, sizeof(p_src_fence->fence.name), "mdp_input_%x_0x%x",
	...
	size = snprintf(p_dst_fence->fence.name, sizeof(p_dst_fence->fence.name), "mdp_output_%x_0x%x",
	...
	*/
	...
	if (mdp_imgresz_get_ticket(pTask, &pTask->task.imgresz_task.ticket)) {
		/* success get a ticket. OK to trigger HW. */
		*need_trigger = 1;
	} else {
		/* HW is busy, wait for HW IRQ, then trigger. */
