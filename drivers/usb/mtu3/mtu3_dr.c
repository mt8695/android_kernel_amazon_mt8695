	...
	int version, bool tohost)
{
	...
	if (version == USB2_PORT) {
		...
		if (tohost)
			mtu3_dual_role_set_host(ssusb->u3d);
		else
			mtu3_dual_role_set_device(ssusb->u3d);
	...
	enum mtu3_vbus_id_state status)
{
	...
	case MTU3_VBUS_OFF:
		mtu3_stop(mtu);
		mtu3_dual_role_set_none(ssusb->u3d);
		pm_relax(ssusb->dev);
		break;
...
static int ssusb_extcon_register(struct otg_switch_mtk *otg_sx)
{
	...
	/* extcon is optional */
	if (!edev) {
		dev_err(ssusb->dev, "-%s since edev = NULL\n", __func__);
		return 0;
	}

	dev_info(ssusb->dev, "EXTCON_USB: %d, EXTCON_USB_HOST: %d\n",
...
static void ssusb_debugfs_init(struct ssusb_mtk *ssusb)
{
	...
	if (otg_sx->default_mode == USB_DR_MODE_PERIPHERAL) {
		/*port0 set default mode as device*/
		ssusb_mode_manual_switch(ssusb, 0);
	} else if (otg_sx->default_mode == USB_DR_MODE_HOST) {
		/*port0 set default mode as host*/
		ssusb_mode_manual_switch(ssusb, 1);
	}

	#ifdef CONFIG_MTU3_SUPPORT_Y_CABLE
	if (!otg_sx->is_y_cable)
		ssusb_mode_manual_switch(ssusb, 0);
	else
		ssusb_mode_manual_switch(ssusb, 1);
	#endif
...
int ssusb_otg_switch_init(struct ssusb_mtk *ssusb)
{
	...
	mtu3_usb_dual_role_init(ssusb->u3d);
	if (otg_sx->manual_drd_enabled) {
		 ...
		 * host initialization
		 */
		schedule_delayed_work(&otg_sx->extcon_reg_dwork, 0);
	}
