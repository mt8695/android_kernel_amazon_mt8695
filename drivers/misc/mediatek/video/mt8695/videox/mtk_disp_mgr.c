...
/*static struct mtk_disp_config config;*/
struct mutex set_input_buffer_ioctl_lock;
...
static long mtk_disp_mgr_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MTK_DISP_IOCTL_SET_INPUT_BUFFER:
		mutex_lock(&set_input_buffer_ioctl_lock);
		memset(pConfig, 0, sizeof(struct mtk_disp_config));

		if (copy_from_user(pConfig, argp, sizeof(struct mtk_disp_config))) {
			vfree(pConfig);
			return -EFAULT;
		}

		ret = mtk_disp_mgr_set_input_buffer(pConfig);

		if (!ret && copy_to_user(argp, pConfig, sizeof(struct mtk_disp_config)))
			ret = -EFAULT;

		mutex_unlock(&set_input_buffer_ioctl_lock);
		break;
...
static int mtk_disp_mgr_probe(struct platform_device *pdev)
{
	...
	mutex_init(&set_input_buffer_ioctl_lock);
	return ret;
}
