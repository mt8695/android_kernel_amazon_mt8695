...
static int mtk_cpu_dvfs_info_init(struct mtk_cpu_dvfs_info *info, int cpu)
{
	...
	proc_reg = devm_regulator_get(cpu_dev, "proc");
	if (IS_ERR(proc_reg)) {
...
static struct cpufreq_driver mt8695_cpufreq_driver = {
...
static int mt8695_cpufreq_probe(struct platform_device *pdev)
{
	struct mtk_cpu_dvfs_info *info;
	...
	ret = cpufreq_register_driver(&mt8695_cpufreq_driver);
	if (ret) {
...
static struct platform_driver mt8695_cpufreq_platdrv = {
	.driver = {
		.name	= "mt8695-cpufreq",
	},
	.probe		= mt8695_cpufreq_probe,
};

static int mt8695_cpufreq_driver_init(void)
{
	...
	err = platform_driver_register(&mt8695_cpufreq_platdrv);
	if (err)
		return err;
	 ...
	 * probing.
	 */
	pdev = platform_device_register_simple("mt8695-cpufreq", -1, NULL, 0);
	if (IS_ERR(pdev)) {
...
device_initcall(mt8695_cpufreq_driver_init);
