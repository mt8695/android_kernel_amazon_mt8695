...
static enum HDR_STATUS _hdr2sdr_write_register(enum WRITE_CMD_ENUM cmd, struct write_cmd_struct *value)
{
	...
	case WRITE_CMD_GET_MODULE_NAME:
		len = snprintf(value->module_name, sizeof(value->module_name), "HDR2SDR");
		if (len < 0) {
			HDR_ERR("link hdr2sdr module name fail");
			memset(value->module_name, 0, sizeof(value->module_name));
		}
