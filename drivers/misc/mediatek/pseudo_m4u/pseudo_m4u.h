...
enum mtk_iommu_callback_ret {
	MTK_IOMMU_CALLBACK_HANDLED,
	MTK_IOMMU_CALLBACK_NOT_HANDLED,
};

/*
 * larb: larb id. 0...MTK_LARB_NR_MAX
 * port: port id within this larb.
 */
typedef enum mtk_iommu_callback_ret
(mtk_iommu_fault_callback_t)(int larb, int port,
			     unsigned int fault_mva, void *cb_data);
...
/* Only for mtk-iommu */
int mtk_iommu_translation_fault_cb(int larb, int port, unsigned int fault_mva);
