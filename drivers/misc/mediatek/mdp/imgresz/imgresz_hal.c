...
void imgresz_hal_set_src_pic_wid_hei(void __iomem *base, uint32_t wid_y, uint32_t wid_c,
	uint32_t wid_cr, uint32_t hei_y, uint32_t hei_c, uint32_t hei_cr)
{
	writel_relaxed(((wid_y & 0xFFFF) << 16) | (hei_y & 0xFFFF),
			       base + RW_IMG_RESZ_SRC_SIZE_Y);
	writel_relaxed(((wid_c & 0xFFFF) << 16) | (hei_c & 0xFFFF),
			       base + RW_IMG_RESZ_SRC_SIZE_CB);
	writel_relaxed(((wid_cr & 0xFFFF) << 16) | (hei_cr & 0xFFFF),
			       base + RW_IMG_RESZ_SRC_SIZE_CR);
}

int imgresz_hal_set_src_pic_offset(void __iomem *base, struct imgresz_scale_data *scaledata)
{
	struct imgresz_buf_format *src_format = &scaledata->src_format;
	unsigned int x_ofs1 = scaledata->src_buf.pic_x_offset;
	unsigned int y_ofs1 = scaledata->src_buf.pic_y_offset;
	unsigned int x_ofs2 = 0, y_ofs2 = 0;
	unsigned int x_ofs3 = 0, y_ofs3 = 0;
	...
	case IMGRESZ_BUF_MAIN_FORMAT_Y_CB_CR:
		if ((src_format->h_sample[1] != 0) && (src_format->v_sample[1] != 0)) {
			...
			x_ofs2 = x_ofs1 / h_sample;
			y_ofs2 = y_ofs1 / v_sample;
		} else {
			x_ofs2 = 0;
			y_ofs2 = 0;
		}
		...
		if ((src_format->h_sample[2] != 0) && (src_format->v_sample[2] != 0)) {
			...
			x_ofs3 = x_ofs1 / h_sample;
			y_ofs3 = y_ofs1 / v_sample;
		} else {
			x_ofs3 = 0;
			y_ofs3 = 0;
		}
	...
	x_ofs1 += scaledata->partition.src_x_offset;
	x_ofs2 += scaledata->partition.src_x_offset_c;

	if (imgresz_cur_chip_ver >= IMGRESZ_CURR_CHIP_VER_8695) {
		writel_relaxed(((x_ofs1 & 0xFFFF) << 16) | (y_ofs1 & 0xFFFF), base + RW_IMG_RESZ_SRC_OFFSET_Y);
		writel_relaxed(((x_ofs2 & 0xFFFF) << 16) | (y_ofs2 & 0xFFFF), base + RW_IMG_RESZ_SRC_OFFSET_CB);
		writel_relaxed(((x_ofs3 & 0xFFFF) << 16) | (y_ofs3 & 0xFFFF), base + RW_IMG_RESZ_SRC_OFFSET_CR);
	} else {
		writel_relaxed(((x_ofs1 & 0xFFF) << 12) | (y_ofs1 & 0xFFF), base + RW_IMG_RESZ_SRC_OFFSET_Y);
		writel_relaxed(((x_ofs2 & 0xFFF) << 12) | (y_ofs2 & 0xFFF), base + RW_IMG_RESZ_SRC_OFFSET_CB);
		writel_relaxed(((x_ofs3 & 0xFFF) << 12) | (y_ofs3 & 0xFFF), base + RW_IMG_RESZ_SRC_OFFSET_CR);
	}
	return 0;
}
