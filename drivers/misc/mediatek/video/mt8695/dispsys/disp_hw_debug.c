...
static int disp_hw_debug_trigger_ui(bool support_ion)
{
	...
	pConfig->buffer_info[0].src_fmt = DISP_HW_COLOR_FORMAT_BGRA8888;
	pConfig->buffer_info[0].type = DISP_LAYER_OSD;
	pConfig->buffer_info[0].src.x = 0;
	pConfig->buffer_info[0].src.y = 0;
	pConfig->buffer_info[0].src.width = 1920;
	pConfig->buffer_info[0].src.height = 1080;
	...
	pConfig->buffer_info[0].ion_fd = fd;
...
static int disp_hw_debug_trigger_vdo(unsigned int support_ion)
{
	...
	pConfig->buffer_info[0].src_base_addr = vaStart;
	pConfig->buffer_info[0].src_phy_addr = (void *)paStart;
	pConfig->buffer_info[0].src_fmt = DISP_HW_COLOR_FORMAT_YUV420_BLOCK;
	pConfig->buffer_info[0].type = DISP_LAYER_VDP;
	pConfig->buffer_info[0].is_progressive = 1;
	if (support_ion == 2) {
		pConfig->buffer_info[0].is_10bit = 1;
		DISP_LOG_I("is_10bit = %d\n", pConfig->buffer_info[0].is_10bit);
	}

	if (support_ion == 3) {
		pConfig->buffer_info[0].src_fmt = DISP_HW_COLOR_FORMAT_YUV420_RASTER;
		pConfig->buffer_info[0].dr_range = DISP_DR_TYPE_DOVI;
		DISP_LOG_I("dr_range = %d\n",
		pConfig->buffer_info[0].dr_range);
	}

	pConfig->buffer_info[0].src.x = 0;
	pConfig->buffer_info[0].src.y = 0;
	pConfig->buffer_info[0].src.width = hw_info.resolution->width;
	pConfig->buffer_info[0].src.height = hw_info.resolution->height;
	pConfig->buffer_info[0].src.pitch = hw_info.resolution->width;
	pConfig->buffer_info[0].tgt.x = 0;
	pConfig->buffer_info[0].tgt.y = 0;
	pConfig->buffer_info[0].tgt.width = hw_info.resolution->width;
	pConfig->buffer_info[0].tgt.height = hw_info.resolution->height;
	pConfig->buffer_info[0].ofst_c = 0x200000;
	pConfig->buffer_info[0].alpha_en = 0;
	pConfig->buffer_info[0].ion_fd = fd;
	pConfig->buffer_info[0].acquire_fence_fd = -1;
...
static int disp_hw_debug_trigger_pip(bool support_ion)
{
	...
	pConfig->buffer_num = 2;
	for (i = 0; i < pConfig->buffer_num; i++) {
		pConfig->buffer_info[i].layer_id = i;
		pConfig->buffer_info[i].layer_enable = 1;
		pConfig->buffer_info[i].src_base_addr = vaStart;
		pConfig->buffer_info[i].src_phy_addr = (void *)paStart;
		pConfig->buffer_info[i].src_fmt = DISP_HW_COLOR_FORMAT_YUV420_BLOCK;
		pConfig->buffer_info[i].type = DISP_LAYER_VDP;
		pConfig->buffer_info[i].is_progressive = 1;
		pConfig->buffer_info[i].src.x = 0;
		pConfig->buffer_info[i].src.y = 0;
		pConfig->buffer_info[i].src.width = 1920;
		pConfig->buffer_info[i].src.height = 1080;
		pConfig->buffer_info[i].src.pitch = 1920;
		pConfig->buffer_info[i].tgt.x = 0;
		pConfig->buffer_info[i].tgt.y = 0;
		pConfig->buffer_info[i].tgt.width = hw_info.resolution->width;
		pConfig->buffer_info[i].tgt.height = hw_info.resolution->height;
		pConfig->buffer_info[i].ofst_c = 0x200000;
		pConfig->buffer_info[i].alpha_en = 0;
		pConfig->buffer_info[i].ion_fd = fd;
		pConfig->buffer_info[i].acquire_fence_fd = -1;
	}
...
static int disp_hw_debug_trigger_secure_vdo(unsigned int vdp_id)
{
	...
	struct disp_hw_common_info hw_info;
	KREE_SECUREMEM_HANDLE sec_handle = 0;
	...
	pConfig->buffer_num = 1;
	pConfig->buffer_info[0].layer_id = vdp_id;
	pConfig->buffer_info[0].layer_enable = 1;
	pConfig->buffer_info[0].src_base_addr = 0;
	pConfig->buffer_info[0].src_phy_addr = 0;
	pConfig->buffer_info[0].src_fmt = DISP_HW_COLOR_FORMAT_YUV420_BLOCK;
	pConfig->buffer_info[0].type = DISP_LAYER_VDP;
	pConfig->buffer_info[0].is_progressive = 1;
	pConfig->buffer_info[0].src.x = 0;
	...
	pConfig->buffer_info[0].ofst_c = 0x200000;
	pConfig->buffer_info[0].alpha_en = 0;
	pConfig->buffer_info[0].ion_fd = (int)sec_handle;
	pConfig->buffer_info[0].acquire_fence_fd = -1;
	pConfig->buffer_info[0].secruity_en = true;
	pConfig->buffer_info[0].buffer_size = 0x400000;
	ret = disp_hw_mgr_config(pConfig);
