...
unsigned int osd_read_pre_fence_counter(unsigned int lay_id)
{
	return (unsigned int)atomic_read(&osd_pre_fence_counter[lay_id]);
}

unsigned int osd_read_timeline_counter(unsigned int lay_id)
{
	return (unsigned int)atomic_read(&osd_timeline_counter[lay_id]);
}
unsigned int osd_read_pre_timeline_counter(unsigned int lay_id)
{
	return (unsigned int)atomic_read(&osd_pre_timeline_counter[lay_id]);
}
...
void osd_release_all_fence(unsigned int lay_id, unsigned int fence_count, unsigned int pre_fence_count)
{
	...
	if (pre_inc > 0) {
		OSD_PRINTF(OSD_FENCE_LOG, "rel all pre-fence:lay_id=%d, inc=%d, fence1=%d, fence2=%d!\n",
			lay_id, pre_inc, atomic_read(&osd_pre_fence_counter[lay_id]),
...
void osd_signal_fence(unsigned int lay_id, unsigned int index)
{
	...
	if (osd_timeline[lay_id] != NULL) {
		...
		OSD_PRINTF(OSD_FENCE_LOG, "vsync_cnt=%ld ,fence_inc=%d, fence_cnt=%d, timeline_cnt=%d!\n",
...
void osd_signal_pre_fence(unsigned int lay_id, unsigned int index)
{
	...
	if (osd_pre_timeline[lay_id] != NULL) {
		...
		/*OSDDBG("fence  : value=%d\n", osd_timeline[lay_id]->value);*/
		OSD_PRINTF(OSD_FENCE_LOG, "vsync_cnt=%ld, pre_fence_inc=%d, pre_fence_cnt=%d, pre_timeline_cnt=%d!\n",
