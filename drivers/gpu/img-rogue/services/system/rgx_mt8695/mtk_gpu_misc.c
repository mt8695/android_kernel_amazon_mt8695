...
{
	mfg_counter_show_trace(m);
	return 0;
}

static ssize_t
power_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *f_pos)
{
	char desc[32];
	int len = 0;
	...
	desc[len] = '\0';
	if (strncmp(desc, "start", 5) == 0) {
		mfg_counter_start_trace();
	} else if (strncmp(desc, "stop", 4) == 0) {
		mfg_counter_stop_trace();
	} else {
		int len;
		int period;

		if (sscanf(desc, "%d %d", &period, &len) == 2)
			mfg_counter_set_hist_size(period, len);
	}
...
static const struct file_operations power_proc_fops = {
...
void mtk_mfg_counter_proc_init(struct proc_dir_entry *parent)
{
	proc_create_data("counter", 0, parent, &power_proc_fops, NULL);
}
...
static int install_cmds(void)
{
	...
	if (pentry) {
		...
		proc_create_data("counter", 0, pentry, &power_proc_fops, NULL);
	}
...
{
	if (pLoading)
		*pLoading = 0;//TIDI PVRMTKGetTotalMemory();
	return true;
}
