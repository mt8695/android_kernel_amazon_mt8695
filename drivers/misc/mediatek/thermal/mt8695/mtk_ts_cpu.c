...
#define THERMAL_NAME    "mt8695-thermal"
...
static int tscpu_register_of_thermal(struct platform_device *pdev);
...
static void thermal_reset_and_initial(void)
{
	...
	/* bus clock 66M counting unit is 4*15.15ns* 256 = 15513.6 ms=15.5us */
	THERMAL_WRAP_WR32(0x00000004, TEMPMONCTL1);
	THERMAL_WRAP_WR32(0x00010507, TEMPMONCTL2);	/* filter interval is 1023 * 15.5us ~ 15.86ms */
	...
	THERMAL_WRAP_WR32(0x00000000, TEMPMONIDET0);   /* times for interrupt occurrance */
	THERMAL_WRAP_WR32(0x00000000, TEMPMONIDET1);   /* times for interrupt occurrance */
	THERMAL_WRAP_WR32(0x00000000, TEMPMONIDET2);   /* times for interrupt occurrance */
...
static void thermal_cal_prepare(struct device *dev)
{
	...
	g_adc_cali_en_t = ((calefuse2 >> 27) & 0x1);      /*ADC_CALI_EN_T(1b) *(0x10206184)[27]*/
	if (g_adc_cali_en_t) {
		g_adc_ge_t      = (calefuse1 & 0x000003FF); /*ADC_GE_T [9:0] *(0x10206180)[9:0]*/
		g_adc_oe_t      = ((calefuse1 >> 10) & 0x3FF);/*ADC_OE_T [9:0] *(0x10206180)[19:10]*/
		g_o_vtsmcu1     = ((calefuse2 >> 9) & 0x1FF); /*O_VTSMCU1    (9b) *(0x10206184)[17:9]*/
		g_o_vtsmcu2     = ((calefuse2 >> 18) & 0x1FF);   /*O_VTSMCU2    (9b) *(0x10206184)[26:18]*/
		g_o_vtsmcu3     = (calefuse3 & 0x1FF); /* O_VTSMCU3    (9b) *(0x10206188)[8:0] */
		g_degc_cali     = ((calefuse1 >> 20) & 0x3F);   /*DEGC_cali    (6b) *(0x10206180)[25:20]*/
		g_o_slope_sign  = ((calefuse2 >> 28) & 0x1);   /*O_SLOPE_SIGN (1b) *(0x10206184)[28]*/
		g_o_slope       = ((calefuse1 >> 26) & 0x3F);  /*O_SLOPE      (6b) *(0x10206180)[31:26]*/
		g_id            = ((calefuse2 >> 29) & 0x1);   /*ID           (1b) *(0x10206184)[29]*/
...
static S32 temperature_to_raw_room(U32 ret)
{
	...
	if (g_o_slope_sign == 0) {	/* O_SLOPE is Positive. */
		...
		format_2 = format_1 * (160 + g_o_slope) * 18 / 15;
...
static S32 raw_to_temperature_roomt(U32 ret, enum thermal_sensor_name ts_name)
{
	...
	if (g_o_slope_sign == 0)
		format_4 = ((format_3 * 100) / (160 + g_o_slope));	/* uint = 0.1 deg */
	else
		format_4 = ((format_3 * 100) / (160 - g_o_slope));	/* uint = 0.1 deg */
...
static int tscpu_set_power_consumption_state(void)
{
	...
	tscpu_dprintk("8695 tscpu_set_power_consumption_state Num_of_OPP=%d\n", Num_of_OPP);
...
static void tscpu_reset_thermal(void)
{
	...
	temp |= 0x00210000; /* 1: Reset THERM */
	THERMAL_WRAP_WR32(temp, PERI_GLOBALCON_RST0);

	temp = DRV_Reg32(PERI_GLOBALCON_RST0);
	temp &= 0xFFDEFFFF; /* 0: Not reset THERM */
	THERMAL_WRAP_WR32(temp, PERI_GLOBALCON_RST0);
}
...
static int get_io_reg_base(struct platform_device *pdev)
{
...
#endif
		node_apmix = of_parse_phandle(node, "apmixedsys", 0);
		apmixed_base = of_iomap(node_apmix, 0);
		tscpu_printk("[THERM_CTRL] apmixed_base=0x%lx\n", (unsigned long)apmixed_base);

		apmixed_phy_base = of_get_phys_base(node_apmix);
		...
		node = of_find_compatible_node(NULL, NULL, "mediatek,mt8695-auxadc");
		...
		node = of_find_compatible_node(NULL, NULL, "mediatek,mt8695-infracfg");
		INFRACFG_AO_BASE = of_iomap(node, 0);
		tscpu_printk("[THERM_CTRL] INFRACFG_AO_BASE=0x%lx\n", (unsigned long)INFRACFG_AO_BASE);

		node = of_find_compatible_node(NULL, NULL, "mediatek,mt8695-pericfg");
