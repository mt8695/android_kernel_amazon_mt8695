...
static bool enable_counter/* = true*/;
...
static int mfg_device_probe(struct platform_device *pdev)
{
	...
	if (enable_counter)
		enable_counter = (mfg_counter_init() == 0);

	pr_info("MFG start is mapped %p, scp %p, counter %d\n", mfg_start, scp_start, enable_counter);
