...
/**
 * @phy_index: order in dtsi
 * @phy_enable: whether phy is enable or not in dtsi
 * @phy_mode: works in which mode:
 *		host only, device only or otg mode
 */
struct mtk_op_phy {
...
struct ssusb_mtk {
	...
	u32 uwk_reg_base;
	u32 uwk_vers;
	struct mtk_op_phy *op_phys;

#if IS_ENABLED(CONFIG_DUAL_ROLE_USB_INTF)
	struct dual_role_phy_instance *dr_usb;
	enum mtu3_dr_force_mode dual_role_mode;
...
void mtu3_intr_status_clear(struct mtu3 *mtu);
void mtu3_intr_disable(struct mtu3 *mtu);
