...
static int mtk_ssusb_extcon_proc_show(struct seq_file *s, void *v)
{
	int extcon_usb;
	int extcon_usb_host;
	struct ssusb_mtk *ssusb = s->private;
	...
	seq_puts(s, "\n===mtk extcon usb gpio debug information===\n");
	seq_puts(s, "  |       State     |    ID     |    VBUS\n");
	seq_puts(s, "--+-----------------+----------------------\n");
	seq_puts(s, " O|  [1] USB        |    H      |    H\n");
	seq_puts(s, " T|  [2] NONE       |    H      |    L\n");
	seq_puts(s, " G|  [3] USB-HOST   |    L      |    H\n");
	seq_puts(s, "  |  [4] USB-HOST   |    L      |    L\n");
	seq_puts(s, "--+-----------------+-----------+----------\n");
	seq_puts(s, " D|  [1] USB        |    X      |    X\n");
	seq_puts(s, " R|  [2] USB-HOST   |    X      |    X\n");

	seq_printf(s, "It is on %s role with %s manner!\n",
		ssusb->is_host ? "host" : "device",
		ssusb->otg_switch.manual_drd_enabled ? "DR" : "OTG");

	if (!ssusb->otg_switch.manual_drd_enabled) {
		extcon_usb	    = extcon_get_cable_state_(edev, EXTCON_USB);
		extcon_usb_host = extcon_get_cable_state_(edev, EXTCON_USB_HOST);
		seq_printf(s, "OTG Current: EXTCON_USB_HOST(id): %s, EXTCON_USB(vBus): %s\n",
			extcon_usb_host ? "L" : "H",
			extcon_usb ? "H" : "L");
#if IS_ENABLED(CONFIG_DUAL_ROLE_USB_INTF)
		seq_printf(s, "       : DUAL_ROLE_PROP: %s\n",
			  (ssusb->dual_role_mode == MTU3_DR_FORCE_DEVICE) ? "Device: MODE_UFP, PR_SNK, DR_DEVICE"
			: (ssusb->dual_role_mode == MTU3_DR_FORCE_HOST)   ? "Host  : MODE_DFP, PR_SRC, DR_HOST"
			: "None  : MODE_DFP, PR_SRC, DR_HOST");
#endif
	}
#if IS_ENABLED(CONFIG_DUAL_ROLE_USB_INTF)
	else
		seq_printf(s, "DR  Current: DUAL_ROLE_PROP: %s\n",
			  (ssusb->dual_role_mode == MTU3_DR_FORCE_DEVICE) ? "Device: MODE_UFP, PR_SNK, DR_DEVICE"
			: (ssusb->dual_role_mode == MTU3_DR_FORCE_HOST)   ? "Host  : MODE_DFP, PR_SRC, DR_HOST"
			: "None  : MODE_NONE, PR_NONE, DR_NONE");
#endif
	return 0;
}

static int mtk_ssusb_extcon_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, mtk_ssusb_extcon_proc_show, inode->i_private);
}

static const struct file_operations mtk_ssusb_extcon_proc_fops = {
	.open   = mtk_ssusb_extcon_proc_open,
	.read   = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
