...
#define LEOPARD_PLL_FMAX		(2500UL * MHZ)
#define CON0_LEOPARD_RST_BAR	BIT(24)
...
#define PLL_B(_id, _name, _reg, _pwr_reg, _en_mask, _flags, _pcwbits,	\
		...
		.en_mask = _en_mask,					\
		.flags = _flags,					\
		.rst_bar_mask = CON0_LEOPARD_RST_BAR,			\
		.fmax = LEOPARD_PLL_FMAX,				\
		.pcwbits = _pcwbits,					\
		.pd_reg = _pd_reg,					\
		.pd_shift = _pd_shift,					\
...
static DEFINE_SPINLOCK(leopard_clk_lock);
...
static const char * const ap2wbmcu_parents[] __initconst = {
	"clkxtal",
	"syspll1_d2",
	"univ48m",
	"syspll1_d8",
...
static const char * const hif_parents[] __initconst = {
	"clkxtal",
	"syspll1_d2",
	"univpll1_d2",
	"syspll1_d4",
...
static const char * const sata_parents[] __initconst = {
	"clkxtal",
	"univpll2_d4"
};
...
static const char * const irrx_parents[] __initconst = {
	"clkxtal",
	"syspll4_d16"
};
...
static const char * const gpt10m_parents[] __initconst = {
	"clkxtal",
	"clkxtal_d4"
};
...
static struct mtk_composite infra_muxes[] __initdata = {
	...
	MUX(CLK_INFRA_MUX1_SEL, "infra_mux1_sel", infra_mux1_parents, 0x000, 2,
		2),
};
...
static struct clk_onecell_data *leopard_top_clk_data __initdata;
static struct clk_onecell_data *leopard_pll_clk_data __initdata;

static void __init mtk_clk_enable_critical(void)
{
	if (!leopard_top_clk_data || !leopard_pll_clk_data)
		return;

	clk_prepare_enable(leopard_pll_clk_data->clks[CLK_APMIXED_ARMPLL]);
	clk_prepare_enable(leopard_top_clk_data->clks[CLK_TOP_AXI_SEL]);
	clk_prepare_enable(leopard_top_clk_data->clks[CLK_TOP_MEM_SEL]);
	clk_prepare_enable(leopard_top_clk_data->clks[CLK_TOP_DDRPHYCFG_SEL]);
}
...
static void __init mtk_topckgen_init(struct device_node *node)
{
	...
	leopard_top_clk_data = mtk_alloc_clk_data(CLK_TOP_NR_CLK);

	mtk_clk_register_fixed_clks(top_fixed_clks, ARRAY_SIZE(top_fixed_clks),
		leopard_top_clk_data);
	mtk_clk_register_factors(top_divs, ARRAY_SIZE(top_divs),
		leopard_top_clk_data);
	mtk_clk_register_composites(top_muxes, ARRAY_SIZE(top_muxes), base,
		&leopard_clk_lock, leopard_top_clk_data);
...
CLK_OF_DECLARE(mtk_topckgen, "mediatek,leopard-topckgen",
	mtk_topckgen_init);
...
CLK_OF_DECLARE(mtk_infracfg, "mediatek,leopard-infracfg", mtk_infracfg_init);
...
CLK_OF_DECLARE(mtk_pericfg, "mediatek,leopard-pericfg", mtk_pericfg_init);

static void __init mtk_apmixedsys_init(struct device_node *node)
{
	int r;

	leopard_pll_clk_data = mtk_alloc_clk_data(CLK_APMIXED_NR_CLK);

	mtk_clk_register_plls(node, plls, ARRAY_SIZE(plls),
		leopard_pll_clk_data);
...
CLK_OF_DECLARE(mtk_apmixedsys, "mediatek,leopard-apmixedsys",
	mtk_apmixedsys_init);
