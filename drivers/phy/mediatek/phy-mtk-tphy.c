...
#define EFUSE_RG_USB20_INTR_CAL		GENMASK(28, 24)
#define PA5_RG_USB20_INTR_CAL_VAL(x)	((0x7 & (x)) << 19)
#define PA1_RG_USB20_VRT_VREF_SEL	GENMASK(14, 12)
#define PA5_RG_USB20_VRT_VREF_SEL_VAL(x) ((0x7 & (x)) << 12)
#define PA1_RG_USB20_TERM_VREF_SEL	GENMASK(10,  8)
#define PA5_RG_USB20_TERM_VREF_SEL_VAL(x) ((0x7 & (x)) <<  8)
	...
	struct mtk_phy_instance *instance)
{
	...
	if (instance->hstx_srctrl <= 7)
		calibration_val = instance->hstx_srctrl;
	else
		dev_info(tphy->dev,
			"PHY%d RG_U2_HSTX_SRCTRL(%u) WASN'T UPDATED with (0x%08X).\n",
			instance->index, calibration_val, instance->vrt_vref);
	...
	struct mtk_phy_instance *instance)
{
	...
	if (instance->privatedata && (*((u32 *)instance->privatedata))) {
		intr_cal  = *((u32 *)instance->privatedata);
		intr_cal &= EFUSE_RG_USB20_INTR_CAL;
		intr_cal >>= 5;

		tmp &= ~PA1_RG_USB20_INTR_CAL;
		tmp |= intr_cal;
		writel(tmp, com + U3P_USBPHYACR1);
	} else {
		if (instance->privatedata)
			dev_info(tphy->dev,
				"PHY%d RG_USB20_INTR_CAL EFUSE 0x%08X WASN'T UPDATED with (0x%08X).\n",
				index, intr_cal, *((u32 *)instance->privatedata));
	}

	tmp  = readl(com + U3P_USBPHYACR1);
	if (instance->vrt_vref <= 7) {
		tmp &= ~PA1_RG_USB20_VRT_VREF_SEL;
		tmp |= PA5_RG_USB20_VRT_VREF_SEL_VAL(instance->vrt_vref);
		writel(tmp, com + U3P_USBPHYACR1);
	} else
		dev_info(tphy->dev,
			"PHY%d RG_USB20_VRT_VREF_SEL WASN'T UPDATED with (0x%08X).\n",
			index, instance->vrt_vref);

	tmp  = readl(com + U3P_USBPHYACR1);
	if (instance->term_vref <= 7) {
		tmp &= ~PA1_RG_USB20_TERM_VREF_SEL;
		tmp |= PA5_RG_USB20_TERM_VREF_SEL_VAL(instance->term_vref);
		writel(tmp, com + U3P_USBPHYACR1);
	} else
		dev_info(tphy->dev,
			"PHY%d RG_USB20_TERM_VREF_SEL WASN'T UPDATED with (0x%08X).\n",
			index, instance->vrt_vref);
...
{
	struct mtk_phy_instance *instance = NULL;
	struct mtk_tphy *tphy = NULL;
	struct u2phy_banks *u2_banks = NULL;
	unsigned long flags;
	unsigned int temp;

	if (!phy) {
		dev_err(tphy->dev, "phy is NULL pointer!\n");
		return;
	}

	/* only xhci port0 need extra operation when portsc[9] be operated */
	if (port)
		return;

	instance = phy_get_drvdata(phy);
	tphy     = dev_get_drvdata(phy->dev.parent);
	u2_banks = &instance->u2_banks;

	if (phy->init_count && phy->power_count) {
		spin_lock_irqsave(&tphy->patch_pp_lock, flags);
		dev_info(tphy->dev, "port%i patch    U2PHYDTM1: 0x%08X\n",
						port, readl(u2_banks->com + U3P_U2PHYDTM1));

		temp  = readl(u2_banks->com + U3P_U2PHYDTM1);
		temp &= ~(P2C_RG_VBUSVALID | P2C_RG_AVALID | P2C_RG_BVALID);
		temp |= P2C_RG_SESSEND;
		writel(temp, u2_banks->com + U3P_U2PHYDTM1);

		temp  = readl(u2_banks->com + U3P_U2PHYDTM1);
		temp |= P2C_FORCE_VBUSVALID;
		writel(temp, u2_banks->com + U3P_U2PHYDTM1);

		temp  = readl(u2_banks->com + U3P_U2PHYDTM1);
		temp |= P2C_FORCE_AVALID | P2C_FORCE_BVALID;
		writel(temp, u2_banks->com + U3P_U2PHYDTM1);

		temp  = readl(u2_banks->com + U3P_U2PHYDTM1);
		temp |= P2C_FORCE_SESSEND;
		writel(temp, u2_banks->com + U3P_U2PHYDTM1);

		dev_info(tphy->dev, "port%i patching U2PHYDTM1: 0x%08X\n",
						port, readl(u2_banks->com + U3P_U2PHYDTM1));
		...
		dev_info(tphy->dev, "port%i patched  U2PHYDTM1: 0x%08X\n",
						port, readl(u2_banks->com + U3P_U2PHYDTM1));
		spin_unlock_irqrestore(&tphy->patch_pp_lock, flags);
	} else
		dev_warn(tphy->dev, "WARNNING: To be ignored since %s(0x%p, %i):cond:init.%i, power.%i\n",
				__func__, phy, port, phy->init_count, phy->power_count);
}
EXPORT_SYMBOL_GPL(mtk_phy_patch_pp);
...
static int mtk_tphy_probe(struct platform_device *pdev)
{
	...
	spin_lock_init(&tphy->patch_pp_lock);
	...
	for_each_child_of_node(np, child_np) {
		...
		cell = of_nvmem_cell_get(child_np, "internal-R-data");
		if (IS_ERR(cell)) {
			retval = PTR_ERR(cell);
			dev_err(dev,
				"PHY%d nvmem_cell_get(...,\"internal-R-data\") failed since ret:%i\n",
				port, retval);
			goto put_child;
		}

		instance->privatedata = nvmem_cell_read(cell, &len);
		if (IS_ERR(instance->privatedata)) {
			retval = PTR_ERR(cell);
			instance->privatedata = NULL;
			dev_err(dev,
				"PHY%d nvmem_cell_read(...) failed since ret:%i\n",
				port, retval);
			goto put_child;
		}
		...
		intr_cal &= EFUSE_RG_USB20_INTR_CAL;
		dev_info(dev,
			"PHY%d nvmem_cell_read(...) RG_USB20_INTR_CAL 0x%08X (0x%08X):len:%zu bytes\n",
			port, intr_cal, *((u32 *)instance->privatedata), len);

		nvmem_cell_put(cell);

		ret = of_property_read_u32(child_np, "eye-patten-vrt-vref-sel", &instance->vrt_vref);
		if (ret) {
			dev_err(dev,
				"PHY%d property: eye-patten-vrt-vref-sel failure since ret: %i\n", port, ret);
			goto put_eyepat;
		}
		dev_info(dev, "PHY%d property: VRT_VREF_SEL  = 0x%08X\n", port, instance->vrt_vref);

		ret = of_property_read_u32(child_np, "eye-patten-term-vref-sel", &instance->term_vref);
		if (ret) {
			dev_err(dev,
				"PHY%d property: eye-patten-term-vref-sel property since ret: %i\n", port, ret);
			goto put_eyepat;
		}
		dev_info(dev, "PHY%d property: TERM_VREF_SEL = 0x%08X\n", port, instance->term_vref);

		ret = of_property_read_u32(child_np, "eye-patten-hstx-srctrl", &instance->hstx_srctrl);
		if (ret) {
			dev_err(dev,
				"PHY%d property: eye-patten-hstx-srctrl property since ret: %i\n", port, ret);
			goto put_eyepat;
		}
		dev_info(dev, "PHY%d property: HSTX_SRCTRL   = 0x%08X\n", port, instance->hstx_srctrl);
	}

	provider = devm_of_phy_provider_register(dev, mtk_phy_xlate);

#ifdef CONFIG_DEBUG_FS
	dev_info(dev, "====mtk_tphy_probe nphys %i in u3phy 0x%p ===\n", tphy->nphys, tphy);
	tphy_phy_init_debugfs(tphy);
#endif
...
put_child:
	port = 0;
	for_each_child_of_node(np, child_np) {
		instance = tphy->phys[port];

		if (instance && instance->privatedata)
			kfree(instance->privatedata);
